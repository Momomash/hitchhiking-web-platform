...
[click] Дальше подключается бизнес, и гонка за релизами. 
...
<!--
И Interop в этой истории не один. Отступим на шаг: перед вами не история фич, а история того, как индустрия училась договариваться.

Слева точка отсчёта — там совместимость достигалась тем, что сайт нюхал User-Agent. Дальше living standard вместо версий раз в пять лет. Test262 и Web Platform Tests — то, что у нас называется CI: общий прогон, по которому видно, кто что не реализовал.

Baseline в 2023-м — единый ответ на «этим уже можно пользоваться»: limited, newly available и widely available через тридцать месяцев. Именно поэтому у одной фичи в докладе бывает две разные даты, и обе честные. А WinterTC — та же схема, но для серверных рантаймов.

Способность договариваться не была дана вебу изначально. Её собрали за тридцать лет, по одному механизму.
